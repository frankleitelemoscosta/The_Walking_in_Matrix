...
<p style = "text-align = justify">
Later the compiler opens the data file again and reads all matrix wrote in this file, 
and this may seem repetitive and wrong, but not, because at the time of writing the 
matrix in the data file only one matrix is ​​used for write of all matrix elements of all matrix wrote in 
the file, and is added more of one matrix into file, and the matrix will be with the
last matrix generated by the program and added the file. When the file is open the
program read all matrix and write it to a vector, that will use for filling in matrix
